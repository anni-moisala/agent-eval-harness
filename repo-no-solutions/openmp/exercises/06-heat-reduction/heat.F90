  ...
  do it = 1, niter
    ...
    unew => tmp
     ...
     if (mod(it, 100) == 0) then
       ...
       !$omp target map(tofrom: avg1) reduction(+:avg1)
       !$omp teams distribute parallel do collapse(2)
       do j = 1, nx2
         do i = 1, ny2
           avg1 = avg1 + u(i,j)
       ...
       !$omp target map(tofrom: avg2) reduction(+:avg2)
       !$omp teams distribute parallel do collapse(2)
       do j = nx2 + 1, nx
         do i = 1, ny2
           avg2 = avg2 + u(i,j)
       ...
       !$omp target map(tofrom: avg3) reduction(+:avg3)
       !$omp teams distribute parallel do collapse(2)
       do j = 1, nx2
         do i = ny2 + 1, ny
           avg3 = avg3 + u(i,j)
       ...
       !$omp target map(tofrom: avg4) reduction(+:avg4)
       !$omp teams distribute parallel do collapse(2)
       do j = nx2 + 1, nx
         do i = ny2 + 1, ny
           avg4 = avg4 + u(i,j)
